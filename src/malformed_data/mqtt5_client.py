...
# Allow malformed module to be executed individually
import sys, os
current_module = 'malformed_data.py'
_, exec_module = os.path.split(sys.argv[0])
if (exec_module == current_module):
    from paho_custom_functions import CustomProperties, _filter_wildcard_len_check
else:
    from .paho_custom_functions import CustomProperties, _filter_wildcard_len_check
...
def init_mqtt5_client(host, port, keepalive, test_name, cID="", clean=1, conn_properties=None, 
                      ...
                      credentials=None, cert_key_paths=None, malformed=False):
    ...
    #If the client wants to send malformed subscriptions the checks must be overloaded
    if(malformed):
        mqtt.Client._filter_wildcard_len_check = _filter_wildcard_len_check
...
def gen_properties(packetType, properties_dict, malformed=False, duplicate=False):
    ...
    """
    # If the properties to send are malformed Properties class has to be overloaded avoiding the Paho checks
    if(not malformed):
        properties = Properties(packetType)
    else:
        properties = CustomProperties(packetType, duplicate)

    for key in properties_dict.keys():
        setattr(properties, key, properties_dict[key])
    return properties

def send_property(host, port, properties_dict, packet_type, credentials=None, cert_key_paths=None, malformed=False, duplicate=False):
    ...
    if(packet_type == PacketTypes.CONNECT or wait_for_event(client, Mqtt5Client.CONNECTION)):
        ...
        if(packet_type == PacketTypes.CONNECT):
            client.connect(host, port, keepalive=10, bind_address="", clean_start=True,
                            properties=gen_properties(packet_type, properties_dict, malformed, duplicate))
            client.loop_start()
            event = Mqtt5Client.CONNECTION
            wait_for_event(client, Mqtt5Client.CONNECTION)
        elif(packet_type == PacketTypes.PUBLISH):
            client.publish("topic/test", "Ciao", qos=2, properties=gen_properties(packet_type, properties_dict, malformed, duplicate))
            event = Mqtt5Client.PUBLISH
        elif(packet_type == PacketTypes.SUBSCRIBE):
            client.subscribe("topic/test", properties=gen_properties(packet_type, properties_dict, malformed, duplicate))
            event = Mqtt5Client.SUBSCRIBE
        elif(packet_type == PacketTypes.UNSUBSCRIBE):
            client.subscribe("topic/test")
            client.unsubscribe("topic/test", properties=gen_properties(packet_type, properties_dict, malformed, duplicate))
            event = Mqtt5Client.UNSUBSCRIBE
        elif(packet_type == PacketTypes.DISCONNECT):
            client.disconnect(properties=gen_properties(packet_type, properties_dict, malformed, duplicate))
